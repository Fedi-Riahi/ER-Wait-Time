...
RAW_PATH       = "data/raw/er_wait_time_dataset.csv"
PROCESSED_PATH = "data/processed/er_clean.csv"


def load_raw() -> pd.DataFrame:
    """Load the raw CSV dataset"""
    if not os.path.exists(RAW_PATH):
        print(f"[DataLoader] '{RAW_PATH}' not found — generating synthetic data")
        return _generate_synthetic()

    df = pd.read_csv(RAW_PATH)
    print(f"[DataLoader] Loaded {len(df):,} rows × {len(df.columns)} columns")
    return df


def load_from_bytes(file_bytes: bytes) -> pd.DataFrame:
    """
    Load dataset from uploaded file bytes
    """
    try:
...
def load_processed() -> pd.DataFrame:
    """Load the cleaned dataset (or raw if processed doesn't exist)."""
...
def _generate_synthetic(n: int = 3000) -> pd.DataFrame:
    """Fallback synthetic data if no CSV provided."""
    np.random.seed(42)
