...
@router.get("/runs")
def get_runs():
    """Return all MLflow runs — used by Experiments page."""
    exp_id = _get_exp_id()
    if not exp_id:
        return []
...
@router.get("/summary")
def get_summary():
    """Return KPI summary — used by Dashboard page."""
    runs = get_runs()
    if not runs:
        return {
            "total_runs": 0,
...
def get_comparison():
    """Return comparison table — used by Comparison page"""
    import pandas as pd
    csv_path = os.path.join(
        os.path.dirname(__file__), '..', '..', 'models', 'comparison_table.csv'
    )
